        ...
        >
            <div className="flex justify-center">
                <div className="bg-slate-900/50 p-1.5 rounded-2xl border border-white/5 flex gap-2">
                    <button 
                        onClick={() => setTab('NEW')}
                        className={`px-8 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${tab === 'NEW' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        New Payload
                    </button>
                    <button 
                        onClick={() => { setTab('HISTORY'); fetchHistory(); }}
                        className={`px-8 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${tab === 'HISTORY' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        History Archive
                    </button>
                </div>
            ...
            {tab === 'NEW' ? (
                ...
                >
                    ...
                    <div className="mt-8 flex items-center gap-4 px-6 py-3 rounded-2xl bg-indigo-500/5 border border-indigo-500/20 text-[11px] font-bold text-indigo-300 uppercase tracking-widest backdrop-blur-sm">
                ...
                <div className="flex-1 bg-slate-900/20 border border-white/5 rounded-[3.5rem] overflow-hidden flex flex-col p-10">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <button 
                            ...
                            className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-900/20 flex items-center gap-3 border border-emerald-500/20 active:scale-95"
                        >
                    ...
                    <div className="flex-1 overflow-auto custom-scrollbar pr-2">
                        <table className="w-full text-left border-separate border-spacing-y-3">
                            <thead>
                                <tr className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em] italic">
                            ...
                            <tbody>
                                {history.map((h) => (
                                    <tr key={h.id} className="group hover:bg-white/[0.02] transition-colors">
                                        <td className="px-6 py-4 bg-slate-900/50 rounded-l-2xl border-y border-l border-white/5 font-mono text-[11px] text-indigo-400">
                                            {h.id.slice(0, 12)}...
                                        </td>
                                        <td className="px-6 py-4 bg-slate-900/50 border-y border-white/5 font-black text-xs text-white uppercase tracking-tight">
                                            {h.fileName}
                                        </td>
                                        <td className="px-6 py-4 bg-slate-900/50 border-y border-white/5 text-[11px] text-slate-500 font-bold">
                                        ...
                                        <td className="px-6 py-4 bg-slate-900/50 rounded-r-2xl border-y border-r border-white/5 text-right">
                                            <button 
                                                onClick={() => logger.info(`Re-queueing job ${h.id}`)}
                                                className="px-3 py-1.5 bg-indigo-500/10 hover:bg-indigo-500 text-indigo-400 hover:text-white rounded-lg text-[11px] font-black uppercase tracking-widest transition-all border border-indigo-500/20 hover:border-indigo-500"
                                    ...
                                    <tr>
                                        <td colSpan={5} className="py-32 text-center">
                                            <div className="flex flex-col items-center gap-6 opacity-20">
                                                ...
                                                <p className="text-[11px] font-black uppercase tracking-[0.4em]">Vault Registry Empty</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
...
function EntityIcon({ icon: Icon, label }: { icon: React.ComponentType<{ className?: string }>, label: string }) {
    return (
        <div className="flex flex-col items-center gap-5 group cursor-default">
            ...
            <span className="text-[11px] font-black text-slate-700 uppercase tracking-[0.3em] italic group-hover:text-slate-400 transition-colors">{label}</span>
        </div>
    );
}
