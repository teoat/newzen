...
export function IntegrateStep({
    ...
    isNotarizing, notarizedBatches, resetAll, projectId
}: IntegrateStepProps) {
    return (
        <motion.div 
            key="step-integrate" 
            initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }}
            className="flex-1 flex flex-col items-center p-20 relative overflow-hidden overflow-y-auto custom-scrollbar"
        >
            ...
            {balanceCheckResult && (
                <div className={`mb-10 px-8 py-4 rounded-2xl border flex items-center gap-6 relative z-10 animate-in slide-in-from-top-4 ${balanceCheckResult.matched ? 'bg-emerald-500/5 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/5 border-rose-500/20 text-rose-400'}`}>
                    ...
                    <div className="space-y-0.5">
                        <p className="text-[11px] font-black uppercase tracking-widest leading-none mb-1">Balance Integrity: {balanceCheckResult.matched ? 'Verified' : 'Failed'}</p>
                        <p className="text-xs font-bold leading-none">{balanceCheckResult.message}</p>
                    </div>
                </div>
            )}

            <p className="text-slate-500 font-bold uppercase tracking-[0.5em] mb-16 max-w-2xl text-center leading-relaxed relative z-10 text-[11px]">
                    ...
                    <div key={fileId} className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] p-10 flex flex-col gap-10 backdrop-blur-xl">
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 mt-1">
                                        <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">{metrics.ingestion_type} Stream</span>
                                        {metrics.ingestionId && (
                                            <button 
                        ...
                        <div className="grid grid-cols-2 gap-10">
                            <div className="space-y-6">
                                <h5 className="text-[11px] font-black text-indigo-400 uppercase tracking-[0.3em] flex items-center gap-3 italic">
                                ...
                                <div className="p-8 bg-black/40 rounded-[2rem] border border-white/5 space-y-6 shadow-inner">
                                    <div className="flex items-start gap-4">
                                        <div className="w-8 h-8 rounded-full bg-indigo-600/20 border border-indigo-500/20 flex items-center justify-center text-[11px] font-black text-indigo-400 shrink-0">P</div>
                                        <div>
                                            <p className="text-[11px] font-black text-white uppercase tracking-tight mb-1">Primary Theory</p>
                                    ...
                                    <div className="flex items-start gap-4">
                                        <div className="w-8 h-8 rounded-full bg-amber-600/20 border border-amber-500/20 flex items-center justify-center text-[11px] font-black text-amber-400 shrink-0">A</div>
                                        <div>
                                            <p className="text-[11px] font-black text-white uppercase tracking-tight mb-1">Alternative Theory</p>
                            ...
                            <div className="space-y-6 flex flex-col">
                                <h5 className="text-[11px] font-black text-emerald-400 uppercase tracking-[0.3em] flex items-center gap-3 italic">
            ...
            <div className="flex flex-col items-center gap-8 relative z-10 w-full max-w-4xl">
                <h4 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-3 italic">
                    <div className="w-12 h-px bg-slate-800" /> Protocol Escalation <div className="w-12 h-px bg-slate-800" />
                </h4>
                
                <div className="grid grid-cols-3 gap-6 w-full">
                    <Link href="/reconciliation?mode=auto_match" className="flex-1">
                        <button className="w-full p-8 rounded-[2.5rem] bg-indigo-600/10 border border-indigo-500/30 hover:bg-indigo-600 hover:text-white transition-all group flex flex-col items-center text-center gap-4 shadow-xl shadow-indigo-900/10 active:scale-95">
                            ...
                            <div>
                                ...
                                <span className="text-[11px] text-slate-500 group-hover:text-indigo-100 font-bold uppercase tracking-tight">Handshake Internal Ledger</span>
                            </div>
                        </button>
                    </Link>
                    
                    <Link href="/forensic/lab?focus=rab_variance" className="flex-1">
                        <button className="w-full p-8 rounded-[2.5rem] bg-emerald-600/5 border border-emerald-500/20 hover:bg-emerald-600 hover:text-white transition-all group flex flex-col items-center text-center gap-4 shadow-xl shadow-emerald-900/10 active:scale-95">
                            ...
                            <div>
                                ...
                                <span className="text-[11px] text-slate-500 group-hover:text-emerald-100 font-bold uppercase tracking-tight">Trace Budget Leakage</span>
                            </div>
                        </button>
                    </Link>
                    ...
                    <Link href="/forensic/nexus" className="flex-1">
                        <button className="w-full p-8 rounded-[2.5rem] bg-amber-600/5 border border-amber-500/20 hover:bg-amber-600 hover:text-white transition-all group flex flex-col items-center text-center gap-4 shadow-xl shadow-amber-900/10 active:scale-95">
                            ...
                            <div>
                                ...
                                <span className="text-[11px] text-slate-500 group-hover:text-amber-100 font-bold uppercase tracking-tight">Map Suspect Nodes</span>
                            </div>
                        </button>
                    </Link>
                </div>
            ...
            <div className="flex gap-8 mt-16 relative z-10">
                <button 
                    onClick={resetAll}
                    className="px-12 py-6 bg-slate-900/50 hover:bg-slate-800 text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-[1.8rem] transition-all border border-white/10 shadow-xl backdrop-blur-xl flex items-center gap-3 active:scale-95"
                >
                    <Plus className="w-4 h-4" /> Ingest New Dataset
                </button>
                <Link href="/" className="px-12 py-6 bg-slate-950 hover:bg-indigo-600 text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-[1.8rem] transition-all shadow-2xl shadow-indigo-900/40 active:scale-95 border border-indigo-400/20 flex items-center justify-center gap-3 group">
