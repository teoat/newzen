...
export default function ForensicPageLayout({ 
    ...
    const { activeProject, isLoading: projectLoading, preferredCurrency, setPreferredCurrency } = useProject();

    return (
        <div className="h-screen bg-[#020617] text-slate-300 font-sans flex flex-col overflow-hidden selection:bg-indigo-500/30">
            <CommandBar />
            {/* Standardized Header */}
            <header className={`h-16 px-6 border-b border-white/5 bg-slate-900/40 backdrop-blur-xl flex items-center justify-between shrink-0 z-banner transition-all duration-700 ${isFocusMode ? 'opacity-20 blur-sm pointer-events-none translate-y--2' : ''}`}>
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-2">
                            {activeProject ? (
                                <span className="text-[11px] bg-indigo-500/10 text-indigo-400 px-1.5 py-0.5 rounded border border-indigo-500/20 font-bold uppercase tracking-wider">
                                    Project: {activeProject.name}
                                </span>
                            ) : (
                                <span className="text-[11px] bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-mono">
                                    NO ACTIVE PROJECT
                                </span>
                            )}
                            {subtitle && (
                                <>
                                    ...
                                    <span className="text-[11px] text-slate-500 font-mono tracking-wide uppercase">{subtitle}</span>
                                </>
                            )}
                        </div>
                    </div>
            ...
            <main className="flex-1 overflow-hidden relative">
                ...
                {loading || projectLoading ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/80 z-overlay backdrop-blur-sm">
